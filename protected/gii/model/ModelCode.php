...
class ModelCode extends CCodeModel {
  ...
  public function prepare() {
    ...
    $srcPath         = str_replace("dist", "src", Yii::getPathOfAlias($this->modelPath));

    if (strpos($srcPath, 'protected' . DIRECTORY_SEPARATOR . 'modules') === FALSE) {
      $srcPath = str_replace("protected" . DIRECTORY_SEPARATOR . "models", "models", $srcPath);
    } else {
      $srcPath = str_replace('protected' . DIRECTORY_SEPARATOR . 'modules', "modules", $srcPath);
    }
    ...
    foreach ($tables as $table) {
      ...
      $this->files[] = new CCodeFile(
        $srcPath . '/' . $className . '.php', $this->render($templatePath . '/model.php', $params)
      );
    }
  }
